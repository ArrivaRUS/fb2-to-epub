...
- **Авто-конвертация по папке.** Указываешь одну папку — фоновый агент сам ловит новые файлы и конвертирует их, без ручных запусков. Отдельный файл `*.fb2` / `*.fb2.zip` → рядом появляется `.epub`; папка с книгами (любая вложенность) → рядом создаётся папка-зеркало с суффиксом `-epub`. Исходники не трогаются, повторные срабатывания идемпотентны (актуальное пропускается).
...
- **`python3`** — входит в Xcode Command Line Tools (`xcode-select --install`). Используется только для поиска обложек в сети, без сторонних зависимостей.
...
- кладёт скрипты агента — `installer.sh`, `fb2-to-epub-runner.sh`, watcher и cover-finder — в `Contents/Resources`;
