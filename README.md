...
Автоматическая конвертация **FB2 и FB3 → EPUB** на macOS по выбранной папке. Установил приложение, указал папку — дальше просто кидаешь в неё `.fb2` / `.fb2.zip` / `.fb3` / папки с книгами, и рядом появляются готовые `.epub`. Установка и статус — в аккуратном нативном окне; сама конвертация идёт фоном, без ручных запусков.
