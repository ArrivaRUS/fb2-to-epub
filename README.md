...
**Приложение само это заметит и проведёт за руку.** Если агент не может прочитать папку, приложение показывает карточку «Нет доступа к папке»: кнопка **«Открыть настройки и скопировать путь»** открывает нужную панель системных настроек и кладёт точный путь цели в буфер обмена, а **«Проверить снова»** сама подхватывает выданный доступ (карточка исчезает, конвертация продолжается). Ничего вводить вручную не нужно.
...
2. Нажми **«+»**, в открывшемся выборе нажми **Cmd-Shift-G** и вставь путь (он уже в буфере):
   ...
   ```
3. Добавь его и **включи переключатель** у `fb2-to-epub-agent`.
...
> Почему именно бинарь: начиная с macOS 26 система привязывает доступ фонового агента к его **исполняемому Mach-O файлу** — доступ, выданный shell-скрипту, просто не действует. Поэтому цель доступа — маленький собственный бинарь `fb2-to-epub-agent` по фиксированному пути: ему выдаёшь доступ один раз, и он переживает обновления.
>
> Если обновляешься со старой версии: строка `fb2-to-epub-runner.sh` в панели Full Disk Access больше ни на что не влияет (она инертна) — её можно спокойно удалить кнопкой «−».
...
- При появлении новых файлов агент запускает **helper** (`fb2-to-epub-agent` — маленький нативный бинарь, FDA-цель), который запускает **watcher** (`fb2-to-epub-watcher.sh`) дочерним процессом и ждёт его: helper остаётся «ответственным» процессом, к которому macOS относит доступ всей цепочки.
...
- кладёт файлы агента — `installer.sh`, замороженный бинарь `fb2-to-epub-agent` (FDA-цель; только копируется, никогда не пересобирается — байт-страж после codesign), `fb2-to-epub-runner.sh` (легаси, один релиз), watcher, cover-finder, `fb2-to-epub-fb3.py` и `fb2-to-epub-fb3-genre.json` (таблица жанров FB3) — в `Contents/Resources`;
